...
## Development
to run this project in PFP (people for peat) project you can use docker
first to build image and run container use this command :
* docker-compose up -d => this command to build image project and run container
* docker-compose up -d --build => this command to rebuild image project and re-run container

### run for development frontend
* docker-compose run -d npm install => this command to run container npm to install all necessary dependency 
* docker-compose run -d npm [command npm in package.json] => this command to start container with command like in package.json eg. docker-compose run -d npm run watch/dev
* docker-compose run -d composer [command composer] => this command to run container with command composer for eg. docker-compose run -d composer install/required pakages
* docker-compose run -d artisan [artisan command] => this command to run container with command artisan for eg. docker-compose run -d artisan make:controller [controller name]
